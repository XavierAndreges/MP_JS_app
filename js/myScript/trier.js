var PlageBaignadePiscineTrierOptions = ["toilette", "douche", "secours", "eauPotable", "sable", "galet", "beachVolley", "eauDouce"];
var PlageBaignadePiscineSousTypeTrierOptions = ["MiseEau", "Plongee", "Slackline"];


var CanyonsTrierOptions = ["Debutant", "Initie", "Sportif", "Expert"];

var SitesNaturelsTrierOptions = ["Calanque", "Ile", "River", "Massif", "Parc", "Littoral"];
var SitesNaturelsSousTypeTrierOptions = ["Slackline", "MiseEau", "Plongee"];

var MonumentsTrierOptions = ["Architecture", "Monument", "Quartier"];
var ExpositionsMuseesTrierOptions = ["Civilisation", "Historique", "Classique", "Moderne", "Contemporain", "Science"];
var SortirTrierOptions = ["Agenda", "Cafe", "Cinema", "Disco", "Concert", "Festival", "Spectacle"];
var BonsPlansTrierOptions = ["Service", "Transport", "Visite", "Atelier"];
var ShoppingTrierOptions = ["Artisanat", "Mode", "Alimentation", "Cave", "ViePratique", "Loisir", "Culture"];
...
var NoIdeaTrierOptions = ["Calanques", "isUpdate"];
var LoisirsTrierOptions = ["Club", "Guide", "ParcLoisir"];
var DormirTrierOptions = ["ChambreHote", "Gite", "Hotel", "Residence", "AubergeJeunesse"];

var RestosTrierOptions = ["10", "15", "20", "25"];
var sousCritereRestosTrierOptions = ["Bouillabaisse", "Aioli", "vueMer", "ruePietonne", "dimanche", "lundi", "midi", "soir", "takeAway"];
...
function showTrierOptions()
{
    ...
    if (!isTrierOptionsOpened)
    {
        ...
        if (!isTrierOptionsActivated)
        {
            ...
            if (lCurrentTableOrActivity == "Restos")
            {
                setHtmlForBlock("sousCritereRestos");
            }
            else
            if (lCurrentTableOrActivity == "PlageBaignadePiscine")
            {
                setHtmlForBlock("PlageBaignadePiscineSousType");
            }
            else
            if (lCurrentTableOrActivity == "SitesNaturels")
            {
                setHtmlForBlock("SitesNaturelsSousType");
            }
            else
            if (lCurrentTableOrActivity == "Hebergements")
            {
                ...
                var blockCategoryHtml = '<div id="blockCategory" class="trierOptionsSousBlock"></div>';
                ...
                var blockChambreHoteHtml = '<div id="blockChambreHote" class="trierOptionsSousBlock"></div>';
                ...
                var blockDiversHebergementsHtml = '<div id="blockDiversHebergements" class="trierOptionsSousBlock"></div>';
        ...
        switch (lCurrentTableOrActivity)
        {
            ...
            case "Restos" : nbItem += sousCritereRestosTrierOptions.length; break;
                
            case "SitesNaturels" : nbItem += SitesNaturelsSousTypeTrierOptions.length; break;
                
            case "PlageBaignadePiscine" : nbItem += PlageBaignadePiscineSousTypeTrierOptions.length; break;
...
function setHtmlForBlock(_type)
{
    ...
    var lBlockHtml = '<div id="block' + _type + '" class="trierOptionsSousBlock"></div>';
...
function getHtmlForTrierOptions(_type, _currentTableOrActivity)
{
    ...
    if (_currentTableOrActivity == "Parkings" || _currentTableOrActivity == "MoniteursEscalade")
        lWidth = 60;
    
    return '<div class="trierCheckboxLabel" style="width:' +  lWidth + '%;" onclick="javascript:trierListItemsParOptionsActivity(\'' + _type + '\')">'+
            '<div id="' + _type + 'Checkbox" class="Checkbox"></div>' +
            '<div class="labelCheckBox">' + _lLabel + '</div>' +
            '</div>'
}
...
function getActivatedCriterias()
{
    ...
    if (_lCurrentTableOrActivity == "Restos")
    {
        addItemIfTypeIsOn("sousCritereRestos", _lCurrentTableOrActivity, lTempArray);
    }
    ...
    if (_lCurrentTableOrActivity == "SitesNaturels")
    {
        addItemIfTypeIsOn("SitesNaturelsSousType", _lCurrentTableOrActivity, lTempArray);
    }
    ...
    if (_lCurrentTableOrActivity == "PlageBaignadePiscine")
    {
        addItemIfTypeIsOn("PlageBaignadePiscineSousType", _lCurrentTableOrActivity, lTempArray);
    }
...
function setCheckBoxToOfforOn(_type)
{
    ...
    if (lCurrentTableOrActivity == "Monuments" || lCurrentTableOrActivity == "ExpositionsMusees"
        || lCurrentTableOrActivity == "BonsPlans" || lCurrentTableOrActivity == "Shopping"
        || lCurrentTableOrActivity == "Sortir"
        || (lCurrentTableOrActivity == "SitesNaturels" && SitesNaturelsTrierOptions.indexOf(_type) != -1)
        || lCurrentTableOrActivity == "ExpositionsMusees" || lCurrentTableOrActivity == "Canyons"
        || lCurrentTableOrActivity == "Petanque" || lCurrentTableOrActivity == "Dormir"
        || lCurrentTableOrActivity == "Loisirs" || lCurrentTableOrActivity == "NoIdea"
        || lCurrentTableOrActivity == "Campings"
        || (lCurrentTableOrActivity == "Randonnee" && RandonneeTrierOptions.indexOf(_type) != -1)
        || (lCurrentTableOrActivity == "SitesEscalade" && SitesEscaladeTrierOptions.indexOf(_type) != -1)
        || (lCurrentTableOrActivity == "Restos" && RestosTrierOptions.indexOf(_type) != -1))
    {
        loopIntoArrayToSetVarAndCheckBox(window[lCurrentTableOrActivity + "TrierOptions"], _type);
    }
    else
    if (lCurrentTableOrActivity == "Randonnee" && MassifsRandonneeTrierOptions.indexOf(_type) != -1)
    {
        loopIntoArrayToSetVarAndCheckBox(window["MassifsRandonneeTrierOptions"], _type);
    }
    else
    if (lCurrentTableOrActivity == "SitesEscalade" && DiversEscaladeTrierOptions.indexOf(_type) == -1)
    {
...
function loopIntoArrayToSetVarAndCheckBox(_array, _type)
{
    ...
    for (var l = 0; l < _array.length; l++)
    {
        ...
        if (lType == _type)
        {
            if (trierOptionsJSON[lCurrentTableOrActivity][lType] == "off")
                trierOptionsJSON[lCurrentTableOrActivity][lType] = "on";
            else
                trierOptionsJSON[lCurrentTableOrActivity][lType] = "off";
        }
        else
...
function trierListItemsParOptionsActivity(_type)
{
    ...
    //*************************** verify if an option is selected ************************
            ...
            //******** :: check function  when add new catefory :: **********/
    ...
    if (_type && lActivatedCriteriasArray.length > 0)
    {
        ...
        for (var n = 0; n < currentActivityItemList.length; n++)
        {
            ...
            {
                lCouldFindIt = switchForTestCouldFindIt(lActivatedCriteriasArray[t], n);
...
function switchForTestCouldFindItForMyItems(lType, n)
{
    ...
    switch (lCurrentTableOrActivity)
    {
        case "Randonnee" :
        {
            if (lType.indexOf(currentActivityItemList[n].type) != -1)
                lCouldFindIt = true;
            else
...
function checkCriteriasRestos(lType, n)
{
    ...
    if (lType == "Aioli" && (currentActivityItemList[n].soustypeIn && currentActivityItemList[n].soustypeIn.indexOf("Aioli") != -1))
        isCorrect = true;
    else
    if (lType == "Bouillabaisse" && (currentActivityItemList[n].soustypeIn && currentActivityItemList[n].soustypeIn.indexOf("Bouillabaisse") != -1))
        isCorrect = true;
